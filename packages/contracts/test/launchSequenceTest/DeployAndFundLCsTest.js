...
contract('Deploying and funding One Year Lockup Contracts', async accounts => {
  ...
  describe('Deploying LCs', async accounts => {
    ...
    it("ZERO Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(zeroToken.address, B, oneYearFromSystemDeployment, { from: liquityAG })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(zeroToken.address, C, oneYearFromSystemDeployment, { from: liquityAG })
    ...
    it("Anyone can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(zeroToken.address, B, oneYearFromSystemDeployment, { from: E })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(zeroToken.address, C, oneYearFromSystemDeployment, { from: F })
      const LC_C_txReceipt = await web3.eth.getTransactionReceipt(LC_C.transactionHash)
    ...
    it("Direct deployment of LC sets the unlockTime in the LC", async () => {
      ...
      const LC_A = await LockupContract.new(zeroToken.address, A, oneYearFromSystemDeployment, { from: liquityAG })
      const LC_B = await LockupContract.new(zeroToken.address, B, '230582305895235', { from: B })
      const LC_C = await LockupContract.new(zeroToken.address, C, dec(20, 18), { from: E })
    ...
    it("Direct deployment of LC reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      const LCDeploymentPromise_A = LockupContract.new(zeroToken.address, A, nearlyOneYear, { from: liquityAG })
      const LCDeploymentPromise_B = LockupContract.new(zeroToken.address, B, '37', { from: B })
      const LCDeploymentPromise_C = LockupContract.new(zeroToken.address, C, '43200', { from: E })
  ...
  describe('Funding LCs', async accounts => {
    it("ZERO transfer from ZERO deployer to their deployed LC increases the ZERO balance of the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: liquityAG })
      ...
      assert.equal(await zeroToken.balanceOf(LCAddress_D), '0')
      assert.equal(await zeroToken.balanceOf(LCAddress_E), '0')

      // Multisig transfers ZERO to each LC
      await zeroToken.transfer(LCAddress_A, ZEROEntitlement_A, { from: multisig })
      await zeroToken.transfer(LCAddress_B, ZEROEntitlement_B, { from: multisig })
      await zeroToken.transfer(LCAddress_C, ZEROEntitlement_C, { from: multisig })
      await zeroToken.transfer(LCAddress_D, ZEROEntitlement_D, { from: multisig })
      await zeroToken.transfer(LCAddress_E, ZEROEntitlement_E, { from: multisig })

      assert.equal(await zeroToken.balanceOf(LCAddress_A), ZEROEntitlement_A)
      assert.equal(await zeroToken.balanceOf(LCAddress_B), ZEROEntitlement_B)
      assert.equal(await zeroToken.balanceOf(LCAddress_C), ZEROEntitlement_C)
      assert.equal(await zeroToken.balanceOf(LCAddress_D), ZEROEntitlement_D)
      assert.equal(await zeroToken.balanceOf(LCAddress_E), ZEROEntitlement_E)
    })

    it("ZERO Multisig can transfer ZERO to LCs deployed through the factory by anyone", async () => {  
      ...
      // Multisig transfers ZERO to each LC
      await zeroToken.transfer(LCAddress_A, dec(1, 18), { from: multisig })
      await zeroToken.transfer(LCAddress_B, dec(2, 18), { from: multisig })
      await zeroToken.transfer(LCAddress_C, dec(3, 18), { from: multisig })
      await zeroToken.transfer(LCAddress_D, dec(4, 18), { from: multisig })
      await zeroToken.transfer(LCAddress_E, dec(5, 18), { from: multisig })

      assert.equal(await zeroToken.balanceOf(LCAddress_A), dec(1, 18))
      assert.equal(await zeroToken.balanceOf(LCAddress_B), dec(2, 18))
      assert.equal(await zeroToken.balanceOf(LCAddress_C), dec(3, 18))
      assert.equal(await zeroToken.balanceOf(LCAddress_D), dec(4, 18))
      assert.equal(await zeroToken.balanceOf(LCAddress_E), dec(5, 18))
    })

    // can't transfer ZERO to any LCs that were deployed directly
  })
  ...
  describe('Withdrawal attempts on funded, inactive LCs immediately after funding', async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers ZERO to each LC
      await zeroToken.transfer(LC_A.address, ZEROEntitlement_A, { from: multisig })
      await zeroToken.transfer(LC_B.address, ZEROEntitlement_B, { from: multisig })
      await zeroToken.transfer(LC_C.address, ZEROEntitlement_C, { from: multisig })

      assert.equal(await zeroToken.balanceOf(LC_A.address), ZEROEntitlement_A)
      assert.equal(await zeroToken.balanceOf(LC_B.address), ZEROEntitlement_B)
      assert.equal(await zeroToken.balanceOf(LC_C.address), ZEROEntitlement_C)
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawZERO({ from: beneficiary })
    ...
    it("ZERO multisig can't withraw from a LC which it funded", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: liquityAG })
      ...
      // ZERO multisig attempts to withdraw from LCs
      for (LC of LCs) {
        try {
          const withdrawalAttemptTx = await LC.withdrawZERO({ from: multisig })
          assert.isFalse(withdrawalAttemptTx.receipt.status)
        } catch (error) {
          assert.include(error.message, "revert")
        }
    ...
    it("No one can withraw from a LC", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, ZEROEntitlement_A, { from: D })

      // Grab contract objects from deployment tx events
      const LC_A = await th.getLCFromDeploymentTx(deployedLCtx_A)

      // LiquityAG transfers ZERO to the LC
      await zeroToken.transfer(LC_A.address, ZEROEntitlement_A, { from: multisig })
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawZERO({ from: G })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawZERO({ from: H })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawZERO({ from: I })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
