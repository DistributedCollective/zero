  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    if (!withProxy) {
      ...
      it("approve(): reverts when owner param is address(0)", async () => {
        const txPromise = zusdTokenTester.callInternalApprove(ZERO_ADDRESS, alice, dec(1000, 18), {from: bob})
        await assertAssert(txPromise)
      })
    }
    ...
    it('decreaseAllowance(): fails trying to decrease more than previously allowed', async () => {
      await zusdTokenTester.approve(bob, dec(3, 18), { from: alice })
      assert.equal((await zusdTokenTester.allowance(alice, bob)).toString(), dec(3, 18))
      await assertRevert(zusdTokenTester.decreaseAllowance(bob, dec(4, 18), { from: alice }), 'ERC20: decreased allowance below zero')
      assert.equal((await zusdTokenTester.allowance(alice, bob)).toString(), dec(3, 18))
    })
    ...
    if (!withProxy) {
      ...
      it('Initializes DOMAIN_SEPARATOR correctly', async () => {
        assert.equal(await zusdTokenTester.domainSeparator(),
      ...
      const buildPermitTx = async (deadline) => {
        ...
        const digest = getPermitDigest(
          tokenName, zusdTokenTester.address,
      ...
      it('permits and emits an Approval event (replay protected)', async () => {
        ...
        await assertRevert(zusdTokenTester.permit(
          approve.owner, approve.spender, approve.value,
          deadline, v, r, s), 'ZUSD: invalid signature')

        // Check that the zero address fails
        await assertAssert(zusdTokenTester.permit('0x0000000000000000000000000000000000000000',
