...
contract('StabilityPool - Withdrawal of stability deposit - Reward calculations', async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawBTCGainToLoC(): Depositors with equal initial deposit withdraw correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })

      // A, B, C open locs
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: alice, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: bob, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: carol, value: dec(10000, 'ether') })
      ...
      // Defaulter opens LoC with 200% ICR and 10k ZUSD net debt
      ...
      // Check depositors' compounded deposit is 6666.66 ZUSD and BTC Gain is 33.16 BTC
      const txA = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })

      // Grab the BTC gain from the emitted event in the tx log
      const alice_BTCWithdrawn = th.getEventArgByName(txA, 'BTCGainWithdrawn', '_ETH').toString()
      const bob_BTCWithdrawn = th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH').toString()
      const carol_BTCWithdrawn = th.getEventArgByName(txC, 'BTCGainWithdrawn', '_ETH').toString()
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '33166666666666666667'), 10000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '33166666666666666667'), 10000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '33166666666666666667'), 10000)
    })

    it("withdrawBTCGainToLoC(): Depositors with equal initial deposit withdraw correct compounded deposit and BTC Gain after two identical liquidations", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Defaulters open LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      ...
      // Check depositors' compounded deposit is 3333.33 ZUSD and BTC Gain is 66.33 BTC
      const txA = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
      // Grab the BTC gain from the emitted event in the tx log
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '66333333333333333333'), 10000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '66333333333333333333'), 10000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '66333333333333333333'), 10000)
    })

    it("withdrawBTCGainToLoC():  Depositors with equal initial deposit withdraw correct compounded deposit and BTC Gain after three identical liquidations", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Check depositors' compounded deposit is 0 ZUSD and BTC Gain is 99.5 BTC 
      const txA = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(99500, 15)), 10000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(99500, 15)), 10000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(99500, 15)), 10000)
    })

    // --- Identical deposits, increasing liquidation amounts ---
    it("withdrawBTCGainToLoC(): Depositors with equal initial deposit withdraw correct compounded deposit and BTC Gain after two liquidations of increasing ZUSD", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Defaulters open LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: '50000000000000000000' })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(7000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: '70000000000000000000' })
      ...
      // (0.5 + 0.7) * 99.5 / 3
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(398, 17)), 10000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(398, 17)), 10000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(398, 17)), 10000)
    })

    it("withdrawBTCGainToLoC(): Depositors with equal initial deposit withdraw correct compounded deposit and BTC Gain after three liquidations of increasing ZUSD", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Defaulters open LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: '50000000000000000000' })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(6000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: '60000000000000000000' })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(7000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: '70000000000000000000' })
      ...
      // (0.5 + 0.6 + 0.7) * 99.5 / 3
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(597, 17)), 10000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(597, 17)), 10000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(597, 17)), 10000)
    })

    // --- Increasing deposits, identical liquidation amounts ---
    it("withdrawBTCGainToLoC(): Depositors with varying deposits withdraw correct compounded deposit and BTC Gain after two identical liquidations", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // 2 Defaulters open LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '33166666666666666667'), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '66333333333333333333'), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(995, 17)), 100000)
    })

    it("withdrawBTCGainToLoC(): Depositors with varying deposits withdraw correct compounded deposit and BTC Gain after three identical liquidations", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '49750000000000000000'), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '149250000000000000000'), 100000)
    })

    // --- Varied deposits and varied liquidation amount ---
    it("withdrawBTCGainToLoC(): Depositors with varying deposits withdraw correct compounded deposit and BTC Gain after three varying liquidations", async () => {
      // Whale opens LoC with 1m BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(1000000, 18)), whale, whale, { from: whale, value: dec(1000000, 'ether') })
      ...
      Defaulter 1: 207000 ZUSD & 2160 BTC
      Defaulter 2: 5000 ZUSD & 50 BTC
      Defaulter 3: 46700 ZUSD & 500 BTC
      */
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('207000000000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(2160, 18) })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5, 21)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(50, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('46700000000000000000000'), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(500, 'ether') })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '11447463383570366500'), 10000000000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '2610021651454043834000'), 10000000000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '74980885162385912900'), 10000000000)
    })

    // --- Deposit enters at t > 0

    it("withdrawBTCGainToLoC(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 ZUSD.  A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: bob, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: carol, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis, value: dec(10000, 'ether') })
      ...
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
      const txD = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis })

      // Grab the BTC gain from the emitted event in the tx log
      const alice_BTCWithdrawn = th.getEventArgByName(txA, 'BTCGainWithdrawn', '_ETH').toString()
      const bob_BTCWithdrawn = th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH').toString()
      const carol_BTCWithdrawn = th.getEventArgByName(txC, 'BTCGainWithdrawn', '_ETH').toString()
      const dennis_BTCWithdrawn = th.getEventArgByName(txD, 'BTCGainWithdrawn', '_ETH').toString()
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '82916666666666666667'), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '82916666666666666667'), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '82916666666666666667'), 100000)

      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '49750000000000000000'), 100000)
    })

    it("withdrawBTCGainToLoC(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 ZUSD.  A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(100, 'ether') })
      ...
      // Dennis opens a LoC and provides to SP
      await zusdToken.transfer(dennis, dec(10000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: dennis })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, dec(995, 17)), 100000)
    })

    it("withdrawBTCGainToLoC(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 1m BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(1000000, 18)), whale, whale, { from: whale, value: dec(1000000, 'ether') })
      ...
      /* Depositors open locs and make SP deposit:
      Alice: 60000 ZUSD
      Bob: 20000 ZUSD
      Carol: 15000 ZUSD
      */
      ...
      /* Defaulters open locs:
      Defaulter 1:  10000 ZUSD, 100 BTC
      Defaulter 2:  25000 ZUSD, 250 BTC
      Defaulter 3:  5000 ZUSD, 50 BTC
      Defaulter 4:  40000 ZUSD, 400 BTC
      */
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(25000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: '250000000000000000000' })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: '50000000000000000000' })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(40000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(400, 'ether') })
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '419563467492260055900'), 100000000000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '139854489164086692700'), 100000000000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '104890866873065014000'), 100000000000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '131691176470588233700'), 100000000000)
    })

    // --- Depositor leaves ---

    it("withdrawBTCGainToLoC(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 ZUSD.  A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Dennis withdraws his deposit and BTC gain
    ...
    it("withdrawBTCGainToLoC(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      */
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(30000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(300, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: '50000000000000000000' })
      ...
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '122461538461538466100'), 100000000000)
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '182361204013377919900'), 100000000000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '227951505016722411000'), 100000000000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '113975752508361205500'), 100000000000)
    })

    // --- One deposit enters at t > 0, and another leaves later ---
    it("withdrawBTCGainToLoC(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 ZUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct ZUSD deposit and BTC Gain", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '82916666666666666667'), 100000)
      ...
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, '92866666666666666667'), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '92866666666666666667'), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '79600000000000000000'), 100000)
    })
    ...
    it("withdrawBTCGainToLoC(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // await borrowerOperations.openLoC(th._100pct, dec(1, 18), account, account, { from: erin, value: dec(2, 'ether') })
      ...
      // Expect Alice and Bob's BTC Gain to be 100 BTC
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000)
      ...
      // Expect Carol and and Dennis BTC Gain to be 50 BTC
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '49750000000000000000'), 100000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '49750000000000000000'), 100000)
    })
    ...
    it("withdrawBTCGainToLoC(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // 4 Defaulters open LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(100, 'ether') })
    ...
    it("withdrawBTCGainToLoC(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: carol, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: erin, value: dec(10000, 'ether') })
      ...
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
      const txD = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis })
      const txE = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: erin })

      const alice_BTCWithdrawn = th.getEventArgByName(txA, 'BTCGainWithdrawn', '_ETH').toString()
      const bob_BTCWithdrawn = th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH').toString()
      const carol_BTCWithdrawn = th.getEventArgByName(txC, 'BTCGainWithdrawn', '_ETH').toString()
      const dennis_BTCWithdrawn = th.getEventArgByName(txD, 'BTCGainWithdrawn', '_ETH').toString()
      const erin_BTCWithdrawn = th.getEventArgByName(txE, 'BTCGainWithdrawn', '_ETH').toString()
      ...
      //Expect Alice and Bob's BTC Gain to be 1 BTC
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000)

      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '16583333333333333333'), 100000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '33166666666666666667'), 100000)
      assert.isAtMost(th.getDifference(erin_BTCWithdrawn, '49750000000000000000'), 100000)
    })
    ...
    it("withdrawBTCGainToLoC(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the BTC Gain from one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
    ...
    // Expect all depositors withdraw 0 ZUSD and 100 BTC

    it("withdrawBTCGainToLoC(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })

      // A, B, C, D, E, F, G, H open locs
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: alice, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: bob, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: carol, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: erin, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: flyn, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: harriet, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: graham, value: dec(10000, 'ether') })
      ...
      const txD = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: dennis })
      const txE = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: erin })
      const txF = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: flyn })
      const txG = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: graham })
      const txH = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: harriet })
      ...
      const dennis_BTCWithdrawn = th.getEventArgByName(txD, 'BTCGainWithdrawn', '_ETH').toString()
      const erin_BTCWithdrawn = th.getEventArgByName(txE, 'BTCGainWithdrawn', '_ETH').toString()
      const flyn_BTCWithdrawn = th.getEventArgByName(txF, 'BTCGainWithdrawn', '_ETH').toString()
      const graham_BTCWithdrawn = th.getEventArgByName(txG, 'BTCGainWithdrawn', '_ETH').toString()
      const harriet_BTCWithdrawn = th.getEventArgByName(txH, 'BTCGainWithdrawn', '_ETH').toString()
      ...
      /* Expect all BTC gains to be 100 BTC:  Since each liquidation of empties the pool, depositors
      should only earn BTC from the single liquidation that cancelled with their deposit */
      assert.isAtMost(th.getDifference(alice_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(erin_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(flyn_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(graham_BTCWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(harriet_BTCWithdrawn, dec(995, 17)), 100000)
    ...
    // expect d(B) = d0(B)/100
    // expect correct BTC gain, i.e. all of the reward
    it("withdrawBTCGainToLoC(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999999910000000000000'), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })

      assert.equal(await stabilityPool.currentScale(), '0')

      // Defaulter 2 withdraws 9900 ZUSD
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(9900, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(60, 'ether') })
      ...
      // Grab the BTC gain from the emitted event in the tx log
      const alice_BTCWithdrawn = await th.getEventArgByName(txA, 'BTCGainWithdrawn', '_ETH').toString()
      ...
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const bob_BTCWithdrawn = await th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH').toString()

      // Expect Bob to retain 1% of initial deposit (100 ZUSD) and all the liquidated BTC (60 bitcoin)
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedZUSDDeposit(bob)).toString(), '100000000000000000000'), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '59700000000000000000'), 100000)
    ...
    // expect d(B) = d0(B)/100
    // expect correct BTC gain, i.e. all of the reward
    it("withdrawBTCGainToLoC(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('59400000000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(330, 'ether') })
      ...
      /* Expect depositors to retain 1% of their initial deposit, and an BTC gain 
      ...
      */
      ...
      const bob_BTCWithdrawn = await th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH').toString()
      const carol_BTCWithdrawn = await th.getEventArgByName(txC, 'BTCGainWithdrawn', '_ETH').toString()
      const dennis_BTCWithdrawn = await th.getEventArgByName(txD, 'BTCGainWithdrawn', '_ETH').toString()

      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, '54725000000000000000'), 100000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, '109450000000000000000'), 100000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, '164175000000000000000'), 100000)
    })

    // Deposit's BTC reward spans one scale change - deposit reduced by correct amount
    ...
    // expect B gets entire BTC gain from L2
    it("withdrawBTCGainToLoC(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(99999, 17)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(99999, 17)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 ZUSD and the full BTC gain of 100 bitcoin
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedZUSDDeposit(bob)).toString(), dec(1, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 100000000000)
    })
    ...
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire BTC gain from L2
    it("withdrawBTCGainToLoC(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999900000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('59999400000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(600, 'ether') })
      ...
      assert.isAtMost(th.getDifference(bob_BTCWithdrawn, dec(995, 17)), 10000000000)
      assert.isAtMost(th.getDifference(carol_BTCWithdrawn, dec(1990, 17)), 100000000000)
      assert.isAtMost(th.getDifference(dennis_BTCWithdrawn, dec(2985, 17)), 100000000000)
    })

    // A make deposit 10000 ZUSD
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 ZUSD
    // Expect A to withdraw 0 deposit
    it("withdrawBTCGainToLoC(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999999999900000000000'), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
    ...
    /* A make deposit 10000 ZUSD
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 ZUSD, 1 BTC
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 ZUSD, 1 BTC
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 ZUSD, 1 BTC
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 ZUSD, 1 BTC
    expect A, B, C, D each withdraw ~100 Bitcoin
    */
    it("withdrawBTCGainToLoC(): Several deposits of 10000 ZUSD span one scale factor change. Depositors withdraws correct compounded deposit and BTC Gain after one liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999900000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999900000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999900000000000000000'), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(100, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount('9999900000000000000000'), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(100, 'ether') })
      ...
      // 99.5 BTC is offset at each L, 0.5 goes to gas comp
      // Each depositor gets BTC rewards of around 99.5 BTC. 1e17 error tolerance
      assert.isTrue(toBN(alice_BTCWithdrawn).sub(toBN(dec(995, 17))).abs().lte(toBN(dec(1, 17))))
      assert.isTrue(toBN(bob_BTCWithdrawn).sub(toBN(dec(995, 17))).abs().lte(toBN(dec(1, 17))))
      assert.isTrue(toBN(carol_BTCWithdrawn).sub(toBN(dec(995, 17))).abs().lte(toBN(dec(1, 17))))
      assert.isTrue(toBN(dennis_BTCWithdrawn).sub(toBN(dec(995, 17))).abs().lte(toBN(dec(1, 17))))
    })

    it("withdrawBTCGainToLoC(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })

      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: A, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: B, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: C, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: E, value: dec(10000, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(10000, 18)), ZERO_ADDRESS, ZERO_ADDRESS, { from: F, value: dec(10000, 'ether') })
      
      // Defaulters 1-3 each withdraw 24100, 24300, 24500 ZUSD (inc gas comp)
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(24100, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(200, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(24300, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(200, 'ether') })
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(24500, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(200, 'ether') })
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18))
      const txA = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: A })
      const txB = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: B })
      await priceFeed.setPrice(dec(100, 18))
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18))
      const txC = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: C })
      const txD = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: D })
      await priceFeed.setPrice(dec(100, 18))
      ...
      // Attempt withdrawals
      const txE = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: E })
      const txF = await stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: F })
    ...
    it("withdrawBTCGainToLoC(): Large liquidated coll/debt, deposits and BTC price", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })

      // BTC:USD price is $2 billion per BTC
      await priceFeed.setPrice(dec(2, 27));

      const depositors = [alice, bob]
      for (account of depositors) {
        await borrowerOperations.openLoC(th._100pct, dec(1, 36), account, account, { from: account, value: dec(2, 27) })
        await stabilityPool.provideToSP(dec(1, 36), ZERO_ADDRESS, { from: account })
      }

      // Defaulter opens LoC with 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(1, 36)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(1, 27) })

      // BTC:USD price drops to $1 billion per BTC
      ...
      // Grab the BTC gain from the emitted event in the tx log
      const alice_BTCWithdrawn = th.getEventArgByName(txA, 'BTCGainWithdrawn', '_ETH')
      const bob_BTCWithdrawn = th.getEventArgByName(txB, 'BTCGainWithdrawn', '_ETH')
      ...
      // Check BTC gains
      const aliceExpectedBTCGain = toBN(dec(4975, 23))
      const aliceBTCDiff = aliceExpectedBTCGain.sub(toBN(alice_BTCWithdrawn))

      assert.isTrue(aliceBTCDiff.lte(toBN(dec(1, 18))))

      const bobExpectedBTCGain = toBN(dec(4975, 23))
      const bobBTCDiff = bobExpectedBTCGain.sub(toBN(bob_BTCWithdrawn))

      assert.isTrue(bobBTCDiff.lte(toBN(dec(1, 18))))
    })

    it("withdrawBTCGainToLoC(): Small liquidated coll/debt, large deposits and BTC price", async () => {
      // Whale opens LoC with 100k BTC
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(100000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      const depositors = [alice, bob]
      for (account of depositors) {
        await borrowerOperations.openLoC(th._100pct, dec(1, 38), account, account, { from: account, value: dec(2, 29) })
        await stabilityPool.provideToSP(dec(1, 38), ZERO_ADDRESS, { from: account })
      }

      // Defaulter opens LoC with 50e-7 BTC and  5000 ZUSD. 200% ICR
      await borrowerOperations.openLoC(th._100pct, await getOpenLoCZUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: '5000000000000' })
      ...
      const txAPromise = stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txBPromise = stabilityPool.withdrawBTCGainToLoC(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })

      // Expect BTC gain per depositor of ~1e11 wei to be rounded to 0 by the BTCGainedPerUnitStaked calculation (e / D), where D is ~1e36.
      await th.assertRevert(txAPromise, 'StabilityPool: caller must have non-zero BTC Gain')
      await th.assertRevert(txBPromise, 'StabilityPool: caller must have non-zero BTC Gain')
