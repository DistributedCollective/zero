...
contract('ZERO Token', async accounts => {
  ...
  const buildPermitTx = async (deadline) => {
    ...
    const digest = getPermitDigest(
      tokenName, zeroTokenTester.address,
  ...
  it("approve(): reverts when owner param is address(0)", async () => {
    ...
    const txPromise = zeroTokenTester.callInternalApprove(ZERO_ADDRESS, A, dec(100, 18), { from: B })
    await assertRevert(txPromise)
  })
  ...
  it('transfer(): transfer to or from the zero-address reverts', async () => {
    ...
    const txPromiseFromZero = zeroTokenTester.callInternalTransfer(ZERO_ADDRESS, A, dec(100, 18), { from: B })
    const txPromiseToZero = zeroTokenTester.callInternalTransfer(A, ZERO_ADDRESS, dec(100, 18), { from: B })
  ...
  it('Initializes DOMAIN_SEPARATOR correctly', async () => {
    assert.equal(await zeroTokenTester.domainSeparator(),
  ...
  it('permit(): permits and emits an Approval event (replay protected)', async () => {
    ...
    await assertRevert(zeroTokenTester.permit(
      approve.owner, approve.spender, approve.value,
      deadline, v, r, s), 'ZERO: invalid signature')

    // Check that the zero address fails
    await assertRevert(zeroTokenTester.permit('0x0000000000000000000000000000000000000000',
      approve.spender, approve.value, deadline, '0x99', r, s), 'ZERO: invalid signature')
  })
