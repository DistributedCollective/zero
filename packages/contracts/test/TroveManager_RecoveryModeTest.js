...
contract('TroveManager - in Recovery Mode', async accounts => {
  const _1_RBtcer = web3.utils.toWei('1', 'ether')
  const _2_RBtcer = web3.utils.toWei('2', 'ether')
  const _3_RBtcer = web3.utils.toWei('3', 'ether')
  const _3pt5_RBtcer = web3.utils.toWei('3.5', 'ether')
  const _6_RBtcer = web3.utils.toWei('6', 'ether')
  const _10_RBtcer = web3.utils.toWei('10', 'ether')
  const _20_RBtcer = web3.utils.toWei('20', 'ether')
  const _21_RBtcer = web3.utils.toWei('21', 'ether')
  const _22_RBtcer = web3.utils.toWei('22', 'ether')
  const _24_RBtcer = web3.utils.toWei('24', 'ether')
  const _25_RBtcer = web3.utils.toWei('25', 'ether')
  const _30_RBtcer = web3.utils.toWei('30', 'ether')
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1RBTC:150ZUSD, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("checkRecoveryMode(): returns false if TCR stays above CCR", async () => {
    ...
    await borrowerOperations.withdrawColl(_1_RBtcer, alice, alice, { from: alice })
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1RBTC:100ZUSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraZUSDAmount: spDeposit, extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1RBTC:100ZUSD, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraZUSDAmount: dec(20000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
   ...
   */
    ...
    L_RBTC = 16.820475 /6 =  2.8034125 ether
    */
    const L_ZUSDDebt = (await troveManager.L_ZUSDDebt()).toString()
    const L_RBTC = (await troveManager.L_RBTC()).toString()

    assert.isAtMost(th.getDifference(L_ZUSDDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
    assert.isAtMost(th.getDifference(L_RBTC, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  })
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ZUSD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZUSD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ZUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraZUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZUSD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZUSD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZUSD < liquidated debt: causes correct Pool offset and RBTC gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const L_ZUSDDebt_After = (await troveManager.L_ZUSDDebt()).toString()
    const L_RBTC_After = (await troveManager.L_RBTC()).toString()

    assert.equal(L_ZUSDDebt_After, '0')
    assert.equal(L_RBTC_After, '0')
  })

  it("liquidate(), with ICR > 110%, and StabilityPool ZUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    ...
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraZUSDAmount: dec(240, 18), extraParams: { from: carol } })
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ZUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraZUSDAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 RBTC and a debt of 62 ZUSD, and provides 10 ZUSD to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Defaulter opens with 60 ZUSD, 0.6 RBTC
    ...
    // Liquidate defaulter. 30 ZUSD and 0.3 RBTC is distributed uniformly between A, B and C. Each receive 10 ZUSD, 0.1 RBTC
  ...
  it("liquidate(): does not affect the SP deposit or RBTC gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraZUSDAmount: spDeposit, extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 ZUSD, ICR = 120% (lowest one)
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    ...
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraZUSDAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 ZUSD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: B_netDebt, extraParams: { from: dennis } })
    ...
    // Bob re-opens the trove, price 200, total debt 250 ZUSD, ICR = 240% (lowest one)
    const { collateral: B_coll_2, totalDebt: B_totalDebt_2 } = await openTrove({ ICR: toBN(dec(240, 16)), extraParams: { from: bob, value: _3_RBtcer } })
    // Alice deposits ZUSD in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ZUSD from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraZUSDAmount: dec(5000, 16), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraZUSDAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits ZUSD to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1RBTC:90ZUSD, reducing TCR below 150%
    ...
    */
    ...
    Note: Originally Erin trove was liquidate but after changing the parameters it isn't. The numbers make sense but we don't have
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1RBTC:85ZUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1RBTC:100ZUSD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraZUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraZUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraZUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraZUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraZUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraZUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraZUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraZUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraZUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    await openTrove({ ICR: toBN(dec(285, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingRBTCReward(alice))
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingZUSDDebtReward(alice))
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and RBTC gain", async () => {
    ...
    // Check remaining ZUSD Deposits and RBTC gain, for whale and depositors whose troves were liquidated
    ...
    assert.isAtMost(th.getDifference(whale_RBTCGain, th.applyLiquidationFee(liquidatedColl).mul(W_zusdAmount).div(totalDeposit)), 4000)
    assert.isAtMost(th.getDifference(alice_RBTCGain, th.applyLiquidationFee(liquidatedColl).mul(A_zusdAmount).div(totalDeposit)), 4000)
    assert.isAtMost(th.getDifference(bob_RBTCGain, th.applyLiquidationFee(liquidatedColl).mul(B_zusdAmount).div(totalDeposit)), 4000)
    ...
    const total_RBTCinSP = (await stabilityPool.getRBTC()).toString()

    assert.isAtMost(th.getDifference(total_ZUSDinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_RBTCinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or RBTC gain", async () => {
    // Whale provides 400 ZUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraZUSDAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraZUSDAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraZUSDAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check ZUSD and RBTC in Pool  before
    ...
    // Check ZUSD and RBTC in Pool after
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides ZUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraZUSDAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ZUSD in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 ZUSD and system coll 2.3 RBTC
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides ZUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraZUSDAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraZUSDAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraZUSDAmount: A_totalDebt, extraParams: { from: whale } })
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 ZUSD and system coll by 2 RBTC
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ZUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraZUSDAmount: dec(500, 16), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraZUSDAmount: dec(500, 16), extraParams: { from: freddy } })

    // Whale provides 1000 ZUSD to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingZUSDDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingZUSDDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingZUSDDebtReward(carol))

    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingRBTCReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingRBTCReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingRBTCReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 ZUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraZUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
