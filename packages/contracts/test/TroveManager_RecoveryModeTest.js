...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1BTC:100ZUSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
   ...
   */
    ...
    */
    ...
    const L_BTC = (await troveManager.L_BTC()).toString()
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZUSD < liquidated debt: causes correct Pool offset and BTC gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const L_BTC_After = (await troveManager.L_BTC()).toString()
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 BTC and a debt of 62 ZUSD, and provides 10 ZUSD to SP
  ...
  it("liquidate(): does not affect the SP deposit or BTC gain when called on an SP depositor's address that has no trove", async () => {
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingBTCReward(alice))
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and BTC gain", async () => {
    ...
    // Check remaining ZUSD Deposits and BTC gain, for whale and depositors whose troves were liquidated
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or BTC gain", async () => {
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingBTCReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingBTCReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingBTCReward(carol))
