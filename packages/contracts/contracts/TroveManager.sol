...
contract TroveManager is TroveManagerBase, CheckContract {
    ...
    /**
     *  Get its offset coll/debt and RBTC gas comp, and close the trove.
     */
    ...
    function liquidateTroves(uint256 _n) external override {
        ...
        // Move liquidated RBTC and ZUSD to the appropriate pools
    ...
    function getNominalICR(address _borrower) public view override returns (uint256) {
        ...
        uint256 NICR = LiquityMath._computeNominalCR(currentRBTC, currentZUSDDebt);
        return NICR;
    }
    ...
    /// Update borrower's snapshots of L_RBTC and L_ZUSDDebt to reflect the current values
    ...
    function _redistributeDebtAndColl(
         ...
         * error correction, to keep the cumulative error low in the running totals L_RBTC and L_ZUSDDebt:
         *
    ...
    function getRedemptionFeeWithDecay(uint256 _RBTCDrawn) external view override returns (uint256) {
    ...
    function getPendingRBTCReward(address _borrower) public view override returns (uint256) {
        return _getPendingRBTCReward(_borrower);
    }
