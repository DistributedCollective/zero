...
class ModelParams:
    def __init__(self):
        ...
        self.T = 5 # weighting for price change in LoC issuance
        self.F = 5 # weighting for momentum change in LoC issuance
...
# return the innate component of market demand for holding ZERO tokens.  Could be a function of:
...
def get_new_token_price(data, params, redeemed_amount, momentum):
    ...
    # price = (data.loc_issuance[-1] - data.token_demand[-1] - ((A + T) * data.token_price[-1]) + ((B + F) * momentum) - redeemed_amount) * factor 
    price = (data.loc_issuance[-1] - data.innate_token_demand - (A * data.token_price[-1] )  -T + ((B + F) * momentum) - redeemed_amount) * factor 
...
def get_new_loc_issuance(data, params, token_price, momentum ):
    loc_issuance = data.loc_issuance[-1] + params.T*(token_price - 1) + params.F*(momentum)
    if loc_issuance < 0:
        return 0
    else: 
        return loc_issuance
...
def linear_decreasing_BTC_price(last_price, gradient):
    return last_price - gradient

def quadratic_BTC_price(scale, i):
    return scale*(i**2)
...
for i in range(1, 100):
    ...
    # BTC_price = oscillating_BTC_price(500, 10, i)
    # BTC_price = quadratic_BTC_price(10, i)
    # BTC_price = linear_increasing_BTC_price(last_BTC_price, 100)
    # BTC_price = linear_decreasing_BTC_price(last_BTC_price, 1)
