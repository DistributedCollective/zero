...
The amount of ZUSD tokens generated by a new trove is
...
The collateral ratio of the existing troves vary with the bitcoin price $P_t^e$
...
def pending_liquidations(contracts, price_BTC_current):
    ...
    last_ICR = contracts.troveManager.getCurrentICR(last_trove, Wei(price_BTC_current * 1e18))
...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, price_ZERO_current, data, index):
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_ZUSD)*n_troves
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        currentICR = contracts.troveManager.getCurrentICR(account, floatToWei(price_BTC_current)) / 1e18
        amounts = contracts.troveManager.getEntireDebtAndColl(account)
        ...
        if p >= ratio:
            debt_new = price_BTC_current * coll / working_trove['CR_initial']
        ...
        #Another part of the troves are adjusted by adjusting collaterals
            ...
            coll_new = working_trove['CR_initial'] * debt / price_BTC_current
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_BTC, CR_ratio, rational_inattention, price_BTC_current):
    ...
    hints = get_hints_from_amounts(accounts, contracts, active_accounts, quantity_BTC, supply_trove, price_BTC_current)
    coll = floatToWei(quantity_BTC)
    debtChange = floatToWei(supply_trove) + ZUSD_GAS_COMPENSATION
    zusd = get_zusd_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_BTC_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, zusd, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, index):
    ...
    if index <= 0:
        ...
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_ZUSD - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        if supply_trove < MIN_NET_DEBT:
            ...
            quantity_BTC = CR_ratio * supply_trove / price_BTC_current

        issuance_ZUSD_open = issuance_ZUSD_open + rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_BTC, CR_ratio, rational_inattention, price_BTC_current):
...
def redeem_trove(accounts, contracts, i, price_BTC_current):
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, index):
    ...
    if price_ZUSD_current > 1.1 + rate_issuance:
        ...
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_BTC, CR_ratio, rational_inattention):
    ...
    if price_ZUSD_current < 1 - rate_redemption:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          if redemption > 0:
              tx = redeem_trove(accounts, contracts, 0, price_BTC_current)
              if tx:
