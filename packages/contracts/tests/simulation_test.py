...
def test_run_simulation(add_accounts, contracts, print_expectations):
    ...
    with open('tests/simulation.csv', 'w', newline='') as csvfile:
        ...
        datawriter.writerow(['iteration', 'BTC_price', 'price_ZUSD', 'price_ZERO', 'num_troves', 'total_coll', 'total_debt', 'TCR', 'recovery_mode', 'last_ICR', 'SP_ZUSD', 'SP_BTC', 'total_coll_added', 'total_coll_liquidated', 'total_zusd_redempted'])
        ...
        for index in range(1, n_sim):
            ...
            result_liquidation = liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, price_ZERO_current, data, index)
            total_coll_liquidated = total_coll_liquidated + result_liquidation[0]
            return_stability = result_liquidation[1]

            #close troves
            result_close = close_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, index)

            #adjust troves
            [coll_added_adjust, issuance_ZUSD_adjust] = adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, index)

            #open troves
            [coll_added_open, issuance_ZUSD_open] = open_troves(accounts, contracts, active_accounts, inactive_accounts, price_BTC_current, price_ZUSD, index)
            ...
            [BTC_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_ZUSD, SP_BTC] = logGlobalState(contracts)
            ...
            datawriter.writerow([index, BTC_price, price_ZUSD, price_ZERO_current, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_ZUSD, SP_BTC, total_coll_added, total_coll_liquidated, total_zusd_redempted])
