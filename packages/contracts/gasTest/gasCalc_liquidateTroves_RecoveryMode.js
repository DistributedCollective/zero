...
contract("Gas cost tests", async accounts => {
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 bitcoin, withdraw 900 ZUSD
    ...
    // Check initial troves with starting 10E/90ZUSD are still open
    for (account of accounts.slice(101, 111)) {
      assert.isTrue(await sortedTroves.contains(account));
    }
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 bitcoin, withdraw 90 ZUSD
    await th.openTrove_allAccounts(
      accounts.slice(101, 111),
      contracts,
      dec(10, "ether"),
  ...
  it("", async () => {
    ...
    // Check initial troves with starting 10E/90ZUSD, and whale's trove, are still open
    for (account of accounts.slice(101, 111)) {
      assert.isTrue(await sortedTroves.contains(account));
    }
  ...
  it("", async () => {
    ...
    //2 acct opens Trove with 1 bitcoin and withdraw 100 ZUSD
    const _2_Defaulters = accounts.slice(1, 3);
    await th.openTrove_allAccounts(_2_Defaulters, contracts, dec(1, "ether"), dec(60, 18));
