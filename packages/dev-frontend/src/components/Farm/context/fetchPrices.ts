...
const uniswapQuery = (zeroTokenAddress: string, uniTokenAddress: string) => `{
...
export async function fetchPrices(zeroTokenAddress: string, uniTokenAddress: string) {
  const response = await window.fetch("https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v2", {
    ...
    body: JSON.stringify({
      query: uniswapQuery(zeroTokenAddress, uniTokenAddress),
      variables: null
    })
  });
  ...
  if (
    ...
    return { zeroPriceUSD, uniLpPriceUSD };
  }
