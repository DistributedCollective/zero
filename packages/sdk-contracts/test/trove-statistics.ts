...
describe("Trove Statistics View Operations", () => {
  ...
  beforeEach(async () => {
    ...
    testIntegrationFactory = await smock.mock<TestIntegration__factory>("TestIntegration");
  ...
  describe("Get entire Debt and Collateral of borrower", async () => {
    it("should call liquidate trove function with correct number of max troves to liquadte", async () => {
      ...
      expect(troveManager.getEntireDebtAndColl).to.have.been.calledOnceWith(signers[1].address);
    });
  });
  ...
  describe("Get BTC Price", async () => {
    it("should call fetchPrice ", async () => {});
    await testIntegration.testGetBtcPrice(priceFeed.address);
    expect(priceFeed.fetchPrice).to.be.calledOnce;
  });
});
